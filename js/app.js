...
class Navigation {
  ...
  navigateTo(page) {
    ...
    if (selectedPage) {
        ...
        // Remove hidden class from back button
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    // Show the back button only if there is a history to go back to
    if (window.history.length > 1) {
        backBtn.classList.remove("hidden");
    } else {
        backBtn.onclick = () => {
            window.location.href = "/"; // Redirect to home or a default page
        };
    }
});