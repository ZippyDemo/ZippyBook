...
class App {
  constructor() {
    ...
    this.savedLocations = [];
    // Cache for geocoded addresses to coordinates
    this._geoCache = new Map();
    // Combined catalog for explore list (mock + live businesses)
    this.catalog = [];
  ...
  init() {
    ...
    // When featured businesses load, merge them and refresh distance-based lists
    document.addEventListener('featuredBusinesses:ready', () => {
      this.mergeCatalog();
      this.refreshDistanceViews();
    });
  ...
  setupEventListeners() {
    ...
    // Login/Signup buttons (guard if header buttons are not present on this layout)
    ...
    if (currentLocationBtn) {
      currentLocationBtn.addEventListener('click', async () => {
        try {
          ...
          googleMapsService.setMapOnLocation(location);
        } catch (error) {
    ...
    // Explore sort select
    const sortSelect = document.querySelector('#explore-page .sort-select');
    if (sortSelect) {
      sortSelect.addEventListener('change', async () => {
        const val = sortSelect.value;
        if (val === 'distance') {
          await this.refreshDistanceViews();
        } else {
          this.updateExploreListBySort(val);
        }
      });
    }
  ...
  updateExploreListBySort(sortValue = 'recommended') {
    const list = document.querySelector('#explore-page .service-list');
    if (!list) return;
    this.mergeCatalog();
    ...
    switch (sortValue) {
      case 'rating':
        ...
        break;
      case 'price-low':
        items.sort((a, b) => (String(a.price || '').length || 0) - (String(b.price || '').length || 0));
        break;
      case 'price-high':
        items.sort((a, b) => (String(b.price || '').length || 0) - (String(a.price || '').length || 0));
        break;
      // 'distance' handled elsewhere to include geocoding and user location
      default:
        // recommended: by featured then rating
        items.sort((a, b) => (Number(b.featured) - Number(a.featured)) || (b.rating || 0) - (a.rating || 0));
    }
    list.innerHTML = '';
    items.slice(0, 30).forEach(s => list.appendChild(this.createServiceCard(s)));
  }
          ...
          if (window.google && window.google.maps) {
            ...
            geocoder.geocode({ location: this.userLocation }, (results, status) => {
              if (status === 'OK' && results[0]) {
                ...
                // Update nearby services
                this.updateNearbyServices();
                
                this.notifications.show('Location updated to ' + address, 'success');
              }
            });
          } else {
            // If Google Maps API not loaded yet
            if (locationInput) {
              locationInput.value = 'Current location detected';
            }
            this.notifications.show('Location detected', 'success');
          }
        },
        (error) => {
          console.error('Error getting location:', error);
          this.notifications.show('Could not get your location. Please enter it manually.', 'error');
        }
      );
    } else {
      this.notifications.show('Geolocation is not supported by your browser', 'error');
    }
  }
  ...
  updateNearbyServices() {
    ...
    this.mergeCatalog();
    // Sort catalog by distance to user location
    const sorted = this.catalog
      .filter(s => s.coordinates)
      .map(s => ({
        ...s,
        distance: this.calculateDistance(this.userLocation.lat, this.userLocation.lng, s.coordinates.lat, s.coordinates.lng)
      }))
      .sort((a, b) => a.distance - b.distance)
      .slice(0, 6);

    // Update nearby section (Home)
  ...
  // Merge local mock services with live business profiles from FeaturedBusinesses
  mergeCatalog() {
    const live = (window.featuredBusinessesInstance?.businesses || []).map(b => this.transformBusinessToService(b));
    const mock = this.services.map(s => ({ ...s }));
    // Key by a composite to avoid duplicates if IDs overlap across domains
    const map = new Map();
    const keyOf = (s) => `${s.source || 'mock'}:${s.id}`;
    mock.forEach(s => { s.source = s.source || 'mock'; map.set(keyOf(s), s); });
    live.forEach(s => { s.source = s.source || 'live'; map.set(keyOf(s), s); });
    this.catalog = Array.from(map.values());
  }

  // Convert business_profiles row to the service card shape we render
  transformBusinessToService(b) {
    const coords = (b.lat != null && b.lng != null)
      ? { lat: Number(b.lat), lng: Number(b.lng) }
      : null;
    return {
      id: b.id,
      name: b.name || 'Business',
      category: (b.displayCategory || b.category || 'other').toLowerCase(),
      subcategory: b.category || 'Business',
      rating: b.rating || 4.5,
      ratingCount: b.rating_count || 0,
      price: b.price_range || '$$',
      image: b.featured_image_url || b.logo_url || 'https://images.unsplash.com/photo-1497366216548-37526070297c?w=800',
      location: b.address || 'Address not specified',
      coordinates: coords,
      description: b.description || 'Professional services available.',
      amenities: [],
      openingHours: {},
      featured: !!b.featured,
      source: 'live'
  ...
  // Recompute and render distance-based views across the app
  async refreshDistanceViews() {
    if (!this.userLocation) return; // will run after geolocation
    // Ensure all catalog items have coordinates; geocode as needed (best effort)
    await this.ensureCatalogCoordinates();
    this.updateNearbyServices();
    this.updateExploreListSortedByDistance();
  }

  async ensureCatalogCoordinates() {
    if (!(window.google?.maps)) return; // require Maps for geocode
    const geocoder = new google.maps.Geocoder();
    const geocodeOne = (address) => new Promise((resolve) => {
      if (!address) return resolve(null);
      const cached = this._geoCache.get(address);
      if (cached) return resolve(cached);
      geocoder.geocode({ address }, (results, status) => {
        if (status === 'OK' && results?.[0]) {
          const loc = results[0].geometry.location;
          const coords = { lat: loc.lat(), lng: loc.lng() };
          this._geoCache.set(address, coords);
          resolve(coords);
        } else {
          resolve(null);
        }
    ...
    // Geocode a small batch per tick to avoid query limits
    const items = this.catalog.filter(s => !s.coordinates && s.location);
    for (const item of items) {
      // eslint-disable-next-line no-await-in-loop
      const coords = await geocodeOne(item.location);
      if (coords) item.coordinates = coords;
    }
  }

  updateExploreListSortedByDistance() {
    const list = document.querySelector('#explore-page .service-list');
    if (!list || !this.userLocation) return;
    // Build or reuse current cards, sort by distance
    const sorted = this.catalog
      .filter(s => s.coordinates)
      .map(s => ({
        ...s,
      ...
      .sort((a, b) => a.distance - b.distance);
    list.innerHTML = '';
    sorted.slice(0, 30).forEach(s => list.appendChild(this.createServiceCard(s)));
  }
  ...
  toggleMapView(view) {
    ...
    if (view === 'list') {
      ...
      // Also refresh the list sorted by distance when switching to map view
      this.refreshDistanceViews();
    }
  }
  ...
  filterServicesByCategory(category) {
    ...
    if (serviceList) {
      ...
      if (allFilteredServices.length === 0) {
        ...
        allFilteredServices.forEach(service => {
          const serviceCard = this.createServiceCard(service);
    ...
    // Show notification
    this.notifications.show(`Showing ${allFilteredServices.length} ${category} services`, 'info');
  }
  ...
  searchServices(query) {
    ...
    // Show notification
    this.notifications.show(`Search results for "${query}"`, 'info');
  }
  ...
  viewServiceDetail(serviceId) {
        ...
        <div class="service-detail-main">
          ...
          <div class="service-detail-info">
            ...
            <div class="service-actions">
              <button class="btn btn-primary btn-book" onclick="window.location.href='business-inventory-page.html?businessId=${service.id || 1}'">View Shop</button>
    ...
    // Add event listeners
    const cancelBtnDetail = detailPage.querySelector('.btn-cancel');
    if (cancelBtnDetail) {
      cancelBtnDetail.addEventListener('click', () => {
        // Navigate to a safe default page instead of using browser history
        this.navigation.navigateTo('explore');
      });
    }
    ...
    if (bookBtn) {
      bookBtn.addEventListener('click', () => {
        window.location.href = `business-inventory-page.html?businessId=${service.id || 1}`;
      });
    }
  ...
  showBookingForm(service) {
    ...
    // No back button; keep navigation explicit
      ...
      case 'explore':
        ...
        if (serviceList && serviceList.children.length === 0) {
          this.app.services.forEach(service => {
            const serviceCard = this.app.createServiceCard(service);
            serviceList.appendChild(serviceCard);
          });
        }
        break;
...
async function initializeMaps() {
    try {
        ...
        googleMapsService.initializeMap('map-container');
        
        // Initialize explore page map
        googleMapsService.initializeMap('explore-map');

        // Get current location and center maps
        const currentLocation = await googleMapsService.getCurrentLocation();
        googleMapsService.setMapOnLocation(currentLocation);
        
        // Add current location marker
        googleMapsService.addMarker(currentLocation, {
            title: 'Your Location',
            icon: {
        ...
        const nearbyServices = await googleMapsService.searchNearbyPlaces(
            currentLocation,
            'establishment',
            5000
        );
        
        // Handle the nearby services data
        displayNearbyServices(nearbyServices);
    } catch (error) {
...
// Back button functionality removed per request; navigation is handled via explicit route changes