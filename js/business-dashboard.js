...
class BusinessDashboard {
    ...
    setupEventListeners() {
        // Initialize delivery section when it becomes active
        document.addEventListener('dashboard:sectionchange', (e) => {
            if (e.detail.section === 'delivery-section') {
    ...
    initDeliverySection() {
        this.populateActiveDrivers();
        this.populateDeliveryOrders();
        this.setupDeliveryFilters();
        this.setupDeliverySearch();
    }

    populateActiveDrivers() {
        const driversContainer = document.getElementById('activeDriversList');
        ...
        const sampleDrivers = [
            ...
            },
            {
                id: 'driver_2',
                name: 'Sarah Wilson',
                photo: 'images/avatars/avatar-2.svg',
                status: 'busy',
            ...
            },
            {
                id: 'driver_3',
                name: 'Alex Chen',
                photo: 'images/avatars/avatar-3.svg',
                status: 'online',
                location: 'East Side',
                rating: 4.7,
                deliveriesToday: 3,
            ...
            },
            {
                ...
                photo: 'images/avatars/avatar-4.svg',
                status: 'online',
                location: 'West End',
                rating: 4.6,
                deliveriesToday: 4,
        ...
        sampleDrivers.forEach(driver => {
            ...
            driverCard.querySelector('.driver-photo').src = driver.photo;
            driverCard.querySelector('.driver-photo').alt = `${driver.name} photo`;
            driverCard.querySelector('.driver-status').className = `driver-status ${driver.status}`;
            driverCard.querySelector('.driver-name').textContent = driver.name;
            driverCard.querySelector('.driver-location').innerHTML = `<i class="fas fa-map-marker-alt"></i> ${driver.location}`;
            driverCard.querySelector('.driver-rating').innerHTML = `<i class="fas fa-star"></i> ${driver.rating}`;
            driverCard.querySelector('.deliveries-count').textContent = driver.deliveriesToday;
            driverCard.querySelector('.avg-time').textContent = driver.avgTime;
            
            driversContainer.appendChild(driverCard);
        });

        // Update driver count
        const activeCount = sampleDrivers.filter(d => d.status === 'online').length;
        const driverCountEl = document.getElementById('activeDriverCount');
        if (driverCountEl) {
            driverCountEl.textContent = `${activeCount} online`;
        }
    }

    populateDeliveryOrders() {
        const ordersContainer = document.getElementById('deliveryItemsList');
        ...
        const template = document.getElementById('delivery-card-template');
        if (!template) return;
        ...
        sampleOrders.forEach(order => {
            ...
            orderCard.querySelector('.delivery-id').textContent = `#${order.id}`;
            orderCard.querySelector('.delivery-time').textContent = order.time;
            orderCard.querySelector('.delivery-status').textContent = order.status.replace('-', ' ');
            orderCard.querySelector('.delivery-status').className = `delivery-status ${order.status}`;
            orderCard.querySelector('.customer-name').textContent = order.customerName;
            orderCard.querySelector('.address-text').textContent = order.address;
            orderCard.querySelector('.items-text').textContent = `${order.items} • ${order.total}`;
            
            const priorityEl = orderCard.querySelector('.delivery-priority');
            if (order.priority) {
            ...
            const assignBtn = orderCard.querySelector('.btn-assign');
            ...
            } else if (order.status === 'assigned' || order.status === 'in-transit') {
        ...
        this.updateFilterCounts(sampleOrders);
    }
    ...
    updateFilterCounts(orders) {
        ...
        // Update statistics
        document.getElementById('pendingDeliveries').textContent = counts.pending;
        document.getElementById('activeDeliveries').textContent = counts['in-transit'];
        document.getElementById('completedDeliveries').textContent = counts.completed;
    }
    ...
    setupDeliveryFilters() {
        ...
        filterBtns.forEach(btn => {
            btn.addEventListener('click', () => {
                ...
                this.filterDeliveryOrders(status);
            });
        });
    }

    filterDeliveryOrders(status) {
        const orderCards = document.querySelectorAll('.delivery-card');
        
        orderCards.forEach(card => {
            const orderStatus = card.querySelector('.delivery-status').className.split(' ').pop();
    ...
    setupDeliverySearch() {
        ...
        if (searchInput) {
            searchInput.addEventListener('input', (e) => {
                this.searchDeliveryOrders(e.target.value);
            });
        }

        if (sortSelect) {
            sortSelect.addEventListener('change', (e) => {
                this.sortDeliveryOrders(e.target.value);
            });
        }
    }

    searchDeliveryOrders(query) {
        const orderCards = document.querySelectorAll('.delivery-card');
        const searchTerm = query.toLowerCase();
        
        orderCards.forEach(card => {
            const orderId = card.querySelector('.delivery-id').textContent.toLowerCase();
            const customerName = card.querySelector('.customer-name').textContent.toLowerCase();
            const address = card.querySelector('.address-text').textContent.toLowerCase();
    ...
    sortDeliveryOrders(sortBy) {
        ...
        orderCards.sort((a, b) => {
            switch (sortBy) {
                case 'newest':
                    // Sort by time (assuming newer times come first)
                    return a.querySelector('.delivery-time').textContent.localeCompare(
                        b.querySelector('.delivery-time').textContent
                    );
                case 'oldest':
                    return b.querySelector('.delivery-time').textContent.localeCompare(
                        a.querySelector('.delivery-time').textContent
                    );
                case 'priority':
                    const aPriority = a.querySelector('.delivery-priority').style.display !== 'none';
                    const bPriority = b.querySelector('.delivery-priority').style.display !== 'none';
