...
function ensureContainer() {
  ...
  sidebar.innerHTML = `
    <div class="cart-header">
      ...
      <h3>Shopping Cart</h3>
      <button id="cartClose" class="cart-close" aria-label="Close"><i class="fas fa-times"></i></button>
    </div>
...
function proceedToCheckout() {
  ...
  // Minimal generic summary fallback
  const totalPrice = getTotalPrice();
  const lines = cart.map(i => `${i.name} x${i.quantity} - ${formatCurrency(Number(i.price) * Number(i.quantity))}`).join('\n');
