...
class CategoryIntegration {
    ...
    init() {
        ...
        // If user is on the explore page now and no category is checked, show all businesses
        const exploreActive = document.getElementById('explore-page')?.classList.contains('active');
        if (exploreActive) {
        ...
        // Also react when businesses dataset becomes available
        document.addEventListener('businesses-loaded', () => {
            const exploreActive = document.getElementById('explore-page')?.classList.contains('active');
            const anyChecked = document.querySelectorAll('.filter-sidebar input[type="checkbox"]:checked').length > 0;
            if (exploreActive && !anyChecked) {
                this.showAllBusinesses();
            }
        });

        // When user clicks the Explore nav links, auto-show if no category is selected
        const wireNav = (selector) => {
            document.querySelectorAll(selector).forEach(el => {
                el.addEventListener('click', () => {
                    // Defer to after routing toggles classes
                    setTimeout(() => {
                        const active = document.getElementById('explore-page')?.classList.contains('active');
                        const anyChecked = document.querySelectorAll('.filter-sidebar input[type="checkbox"]:checked').length > 0;
                        if (active && !anyChecked) this.showAllBusinesses();
                    }, 0);
                });
            });
        };
        wireNav('.nav-link[data-page="explore"]');
        wireNav('#footer-explore-link');
    }
    ...
    async displayBusinessResults(businesses, category) {
        ...
        // If our data source is mock/filler, avoid showing filler businesses on Explore
        if (window.featuredBusinessesInstance && window.featuredBusinessesInstance.isMockData === true) {
            // If there's already content (e.g., services), don't override with empty-state
            if (!serviceList.children.length) {
                serviceList.innerHTML = `
                    <div class="empty-state">
                        <p>No businesses found in the ${category} category</p>
                        <p>Real businesses will appear here when available.</p>
                    </div>
                `;
            }
            return;
