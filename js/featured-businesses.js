...
class FeaturedBusinesses {
    constructor() {
        ...
        // Track whether we're showing mock (filler) data
        this.isMockData = false;
        // Limit of displayed cards
        this.maxItems = 10;
        // Prevent duplicate swipe handlers
    ...
    async init() {
        ...
        // Notify the app that businesses data is available (or mock fallback)
        try {
            document.dispatchEvent(new CustomEvent('businesses-loaded', {
                detail: { count: this.businesses.length, isMockData: this.isMockData }
            }));
        } catch (_) {}
    ...
    renderCarousel() {
        ...
        // Apply hard limit of 10 items
        const renderList = [...this.businesses].slice(0, this.maxItems);
        ...
        // Track how many we rendered
        this.displayedCount = renderList.length;
    ...
    createIndicators() {
        ...
        const count = this.getDisplayedCount();
        const totalSlides = Math.max(1, count - this.visibleCards + 1);
    ...
    const maxIndex = Math.max(0, this.getDisplayedCount() - this.visibleCards);
    ...
    previousSlide() {
        if (this.currentIndex > 0) {
            ...
            // Go to last slide
            this.currentIndex = Math.max(0, this.getDisplayedCount() - this.visibleCards);
        }
        this.updateCarousel();
    }
    ...
    startAutoRotate() {
        if (this.getDisplayedCount() <= this.visibleCards) return; // Don't auto-rotate if all cards are visible
    ...
    enableTouchSwipe() {
        if (this._swipeBound) return; // avoid duplicate bindings
        const target = this.container || this.carousel;
        if (!target) return;
        let startX = 0, startY = 0, dx = 0, dy = 0, isSwiping = false;
        const threshold = 40; // px
        const onStart = (x, y) => { startX = x; startY = y; dx = 0; dy = 0; isSwiping = false; this.stopAutoRotate(); };
        const onMove = (x, y) => {
        ...
        const onEnd = () => {
            if (isSwiping && Math.abs(dx) > threshold) {
                if (dx < 0) this.nextSlide(); else this.previousSlide();
            }
            this.startAutoRotate();
        };
    ...
    // Helper to know how many cards are currently rendered/displayed
    getDisplayedCount() {
        if (this.carousel && this.carousel.children && this.carousel.children.length) {
            return this.carousel.children.length;
        }
        if (typeof this.displayedCount === 'number') return this.displayedCount;
        return Math.min(this.businesses.length, this.maxItems);
    }
