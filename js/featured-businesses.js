...
class FeaturedBusinesses {
    ...
    async init() {
        ...
        // Notify listeners that featured businesses are ready
        try {
            document.dispatchEvent(new CustomEvent('featuredBusinesses:ready', { detail: { count: this.businesses.length } }));
        } catch (_) { /* noop */ }
    ...
    cacheElements() {
        ...
        this._touch = { startX: 0, deltaX: 0, active: false };
    }
    ...
    formatBusinessData(business) {
        ...
        return {
            ...
            // Optional location fields if present in DB
            lat: business.lat ?? business.latitude ?? null,
            lng: business.lng ?? business.longitude ?? null
        };
    }
    ...
    setupEventListeners() {
        ...
        // Pause auto-rotate on hover (no-op on touch but harmless)
        if (this.container) {
        ...
        if (this.carousel) {
            ...
            this.carousel.addEventListener('touchmove', (e) => {
                ...
                const t = e.touches[0];
                this._touch.deltaX = t.clientX - this._touch.startX;
            }, { passive: true });

            this.carousel.addEventListener('touchend', () => {
                if (!this._touch.active) return;
                const threshold = 40; // px swipe threshold
                if (this._touch.deltaX > threshold) {
                    this.previousSlide();
                } else if (this._touch.deltaX < -threshold) {
                    this.nextSlide();
                }
    ...
    createBusinessCard(business) {
        ...
        card.innerHTML = `
            ...
            <div class="business-info">
                ...
                <div class="business-distance" style="margin-top:8px;color:var(--text-medium);font-size:0.9rem;"></div>
            </div>
        `;
        ...
        // If app has userLocation and business has coords, show distance
        setTimeout(() => {
            try {
                const loc = window.app?.userLocation;
                const lat = business.lat; const lng = business.lng;
                if (loc && lat != null && lng != null) {
                    const km = this.haversineKm(loc.lat, loc.lng, Number(lat), Number(lng));
                    const el = card.querySelector('.business-distance');
                    if (el) el.innerHTML = `<i class="fas fa-route"></i> ${km.toFixed(1)} km away`;
                }
            } catch (_) {}
        }, 0);
    ...
    updateCarousel() {
        ...
        // Compute actual card width including gap for responsive layouts
        const firstCard = this.carousel.querySelector('.business-card');
        let cardWidth = 320;
        if (firstCard) {
            const style = window.getComputedStyle(firstCard);
            const marginRight = parseFloat(style.marginRight || '0');
            cardWidth = firstCard.getBoundingClientRect().width + marginRight;
        }
