...
// Determine if current logged-in user is a business
async function isBusinessUser() {
  try {
    const { data } = await supabase.auth.getUser();
    const user = data?.user;
    if (user) {
      const role = (user.user_metadata?.role || '').toLowerCase();
      if (role) return role === 'business';
    }
  } catch (_) {}
  try {
...
// Determine if the current viewer is a business user looking at their own shop page
async function isViewingOwnShop() {
  try {
    ...
    return String(user.id) === String(bizId);
  } catch (_) {
    return false;
  }
}
...
function proceedToCheckout(e) {
  ...
  if (cart.length === 0) {
    // Block navigation if empty and we're handling the click
    alert('Your cart is empty!');
    return;
  }

  // Navigate to checkout page, keeping current business id in query for return links
...
async function loadShop() {
  ...
  if (inventoryGrid) {
    inventoryGrid.innerHTML = `
      <div class="loading-inventory" style="
        grid-column: 1 / -1;
        text-align: center;
        padding: 3rem 1rem;
        color: #6b7280;
  ...
  // Load inventory with enhanced error handling
  let items = [];
  let hasError = false;
  
  try {
    console.log('Fetching inventory for business:', id);
    const rows = await fetchInventoryByBusinessId(id);
    items = Array.isArray(rows) ? rows : [];
    console.log('Inventory loaded successfully:', items.length, 'items');
  } catch (e) {
    console.error('Failed to fetch inventory:', e?.message || e);
    hasError = true;
    
    // Show user-friendly error message
    showToast('Unable to load latest inventory', 'Showing cached or sample data instead', 'warning', 5000);
  }
  ...
  // Resolve images with better error handling
  const resolved = await Promise.all((items || []).map(async (item) => {
    try {
      return await resolveItemDisplay(item);
    } catch (err) {
      console.warn('Failed to resolve images for item:', item.name, err);
      return item; // Return item without resolved images
    }
  }));
  ...
  if (!resolved || resolved.length === 0) {
    if (grid) {
      if (hasError) {
        ...
        grid.innerHTML = `
          ...
          ">
            ...
            <h3 style="margin: 0 0 0.5rem 0; color: #ef4444;">Unable to Load Inventory</h3>
      ...
      grid.style.display = hasError ? 'grid' : 'none';
    }
    if (empty && !hasError) empty.style.display = '';
    return;
  }
...
function setupCartEventListeners() {
  ...
  if (checkoutBtn) {
    // Ensure fallback href points to the correct URL even if JS fails
    const businessId = getQueryParam('id');
    const url = businessId ? `checkout.html?id=${encodeURIComponent(businessId)}` : 'checkout.html';
    try { checkoutBtn.setAttribute('href', url); } catch (_) {}
    checkoutBtn.addEventListener('click', proceedToCheckout);
  }
}

document.addEventListener('DOMContentLoaded', async () => {
  // Hide cart UI for business users on the shop page
  try {
    const allowCartOverride = /^(1|true|yes)$/i.test(String(getQueryParam('allowCart') || ''));
    const isBiz = await isBusinessUser();
    const ownShop = await isViewingOwnShop();
    // Only hide if the business user is viewing THEIR OWN shop, unless explicitly overridden
    if (!allowCartOverride && isBiz && ownShop) {
