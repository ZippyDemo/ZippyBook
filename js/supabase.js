...
// Enhanced Supabase client with GitHub Pages compatibility
...
if (window.location.hostname.includes('github.io') || window.location.hostname.includes('githubusercontent.com')) {
  console.log('GitHub Pages environment detected, applying Supabase optimizations...');
  supabaseOptions.auth.storage = window.localStorage; // Ensure localStorage is used
  supabaseOptions.auth.storageKey = 'zippybook.auth.token'; // Custom storage key
}
...
// Public: fetch any business profile by ID (does not require being that user)
export async function fetchBusinessProfileById(businessId) {
...
// Public: fetch inventory for a specific business by ID
export async function fetchInventoryByBusinessId(businessId) {
    if (!businessId) throw new Error('businessId required');
    ...
    try {
        ...
        if (error) {
            ...
            // On GitHub Pages, try fallback mechanisms
            if (window.location.hostname.includes('github.io') || window.location.hostname.includes('githubusercontent.com')) {
                console.log('Attempting fallback inventory loading...');
                return await loadInventoryFallback(businessId);
            }
            throw error;
        }
        // If we got results, cache them for fallback use and return
        if (Array.isArray(data) && data.length > 0) {
            try {
                const cacheKey = `zippybook:${businessId}:inventory`;
                localStorage.setItem(cacheKey, JSON.stringify(data));
            } catch (_) {}
            return data;
        }

        // If no results but running on GitHub Pages, attempt fallback (RLS may prevent anon reads)
        const isGhPages = window.location.hostname.includes('github.io') || window.location.hostname.includes('githubusercontent.com');
        if (isGhPages) {
            console.warn('No inventory returned from Supabase; using GitHub Pages fallback');
            return await loadInventoryFallback(businessId);
        }
        ...
        // Fallback for GitHub Pages or network issues
...
// Fallback inventory loading mechanism for GitHub Pages
async function loadInventoryFallback(businessId) {
    try {
        // First, try localStorage cache
        const cacheKey = `zippybook:${businessId}:inventory`;
        const cached = localStorage.getItem(cacheKey);
        if (cached) {
            console.log('Loading inventory from localStorage cache');
        ...
        // If no cache, return sample data for demonstration
        console.log('No cached inventory found, using sample data');
        return getSampleInventoryData(businessId);
    } catch (err) {
        console.error('Fallback inventory loading failed:', err);
        return [];
    }
}

// Sample inventory data for demonstration when Supabase is unavailable
function getSampleInventoryData(businessId) {
    const sampleItems = [
        {
            id: 'sample-1',
            ...
            flags: { isAvailable: true, isVeg: false },
        ...
        },
        {
            ...
            name: 'Veggie Delight',
            price: 10.99,
            description: 'Fresh vegetables with quinoa and house dressing',
            category: 'Salads',
            stock: 15,
            image_url: 'https://images.unsplash.com/photo-1512621776951-a57141f2eefd?w=400&h=300&fit=crop',
            flags: { isAvailable: true, isVeg: true },
            created_at: new Date().toISOString()
        },
        {
            id: 'sample-3',
            ...
            stock: 12,
            image_url: 'https://images.unsplash.com/photo-1532636875304-0c89119d9cc4?w=400&h=300&fit=crop',
            flags: { isAvailable: true, isVeg: false },
            created_at: new Date().toISOString()
        },
        {
