...
// ===== Public fetchers used by shop page and tests =====
// Fetch a business profile by its id (uuid)
export async function fetchBusinessProfileById(id) {
...
// Fetch inventory items for a given business id
// Also attempts to resolve displayable image URLs into `item._display_image`
export async function fetchInventoryByBusinessId(businessId) {
    ...
    // Best-effort resolve main and additional image URLs for convenience
    const resolved = await Promise.all(rows.map(async (item) => {
        const r = { ...item };
        try {
            if (r.image_url) {
                // If it's a storage path, convert to accessible URL; otherwise keep as-is
                r._display_image = await getInventoryFileUrl(r.image_url);
            } else if (r.imageUrl) {
                r._display_image = await getInventoryFileUrl(r.imageUrl);
            } else if (r.imagePath) {
                r._display_image = await getInventoryFileUrl(r.imagePath);
            }
        } catch (_) {}
        try {
            if (Array.isArray(r.additional_image_urls)) {
                r._display_additional = await Promise.all(r.additional_image_urls.map(async (p) => {
